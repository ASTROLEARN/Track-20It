  ...
  useEffect(() => {
    const i = setInterval(() => {
      setRoster((prev) =>
        prev.map((s) =>
          Math.random() > 0.8
            ? { ...s, status: ["present", "absent", "late"][Math.floor(Math.random() * 3)] as Student["status"] }
            : s,
        ),
      );
    }, 1500);
  ...
  return (
    <div className="space-y-16">
      ...
      <section id="features" className="grid gap-6 lg:grid-cols-2">
        <Card className="relative overflow-hidden">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl">
              <ScanQrCode className="h-5 w-5 text-primary" /> Automated Attendance
            </CardTitle>
            <CardDescription>
              Choose the method that fits your infrastructure: QR, Bluetooth/Wi‑Fi proximity, or Face Recognition. Hybrid models prevent proxy attendance.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="qr" className="w-full">
              <TabsList className="mb-4">
                ...
                <TabsTrigger value="bt" className="gap-2"><Bluetooth className="h-4 w-4"/> Bluetooth</TabsTrigger>
                <TabsTrigger value="wifi" className="gap-2"><Wifi className="h-4 w-4"/> Wi‑Fi</TabsTrigger>
                <TabsTrigger value="face" className="gap-2"><ScanFace className="h-4 w-4"/> Face ID</TabsTrigger>
              </TabsList>
              <TabsContent value="qr">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-md border p-4">
                    <p className="text-sm text-muted-foreground">Time‑bound session QR</p>
              ...
              <TabsContent value="bt">
                <ul className="grid gap-2 text-sm sm:grid-cols-2">
                  <FeatureLine>Passive detection via signal proximity</FeatureLine>
                  <FeatureLine>Seamless student experience</FeatureLine>
                  <FeatureLine>Beacon/AP calibration support</FeatureLine>
                  <FeatureLine>Privacy‑first: device identifiers only</FeatureLine>
                </ul>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl"><Users2 className="h-5 w-5 text-primary"/> Real‑time Attendance</CardTitle>
            <CardDescription>Display live presence on classroom screens, kiosks, or TVs.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between rounded-md border p-3 text-sm">
              <div className="font-medium">ENG‑101 • Room A2</div>
              <div className="text-muted-foreground">Present {present}/{total}</div>
            </div>
              ...
              {roster.map((s) => (
                <div
                  ...
                  className={cn(
                    ...
                    s.status === "present" && "bg-emerald-500/10 border-emerald-500/30",
                    s.status === "late" && "bg-amber-500/10 border-amber-500/30",
                    s.status === "absent" && "bg-rose-500/10 border-rose-500/30",
                  )}
                >
                  <span>{s.name}</span>
                  <span className={cn(
                    "inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs",
                    s.status === "present" && "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400",
                    s.status === "late" && "bg-amber-500/15 text-amber-600 dark:text-amber-400",
                    s.status === "absent" && "bg-rose-500/15 text-rose-600 dark:text-rose-400",
                  )}>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl"><Sparkles className="h-5 w-5 text-primary"/> Personalized Free‑Period Tasks</CardTitle>
            <CardDescription>Recommendation engine suggests micro‑tasks aligned with goals and NEP 2020.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {taskChips.map((t, i) => (
                <span key={i} className="rounded-full bg-muted px-3 py-1 text-sm">{t}</span>
              ))}
            </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl"><Clock className="h-5 w-5 text-primary"/> Integrated Daily Routine</CardTitle>
            <CardDescription>Auto‑plan sessions, breaks, and tasks.</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <Button className="mt-4 w-full" variant="outline">Export as PDF</Button>
          </CardContent>
        </Card>
      </section>

      <section className="grid gap-6 lg:grid-cols-4">
        <RoleCard title="Students" desc="See routine, tasks, and attendance history." />
        <RoleCard title="Teachers" desc="Start sessions and view live rosters." />
        <RoleCard title="Administrators" desc="Analytics, compliance & reports." />
      ...
      <section>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-xl"><ShieldCheck className="h-5 w-5 text-primary"/> Privacy‑first and Accessible</CardTitle>
            <CardDescription>Zero‑trust security, consent‑driven biometrics, WCAG 2.1 AA, and offline‑first sync.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap items-center justify-between gap-3 text-sm text-muted-foreground">
            <p>Minimal infrastructure • Basic training • API‑ready for LMS/SIS</p>
            <Link to="/dashboard"><Button>Open Demo Dashboard</Button></Link>
          </CardContent>
        </Card>
      </section>
    </div>
    ...
    <section className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-primary/5 via-background to-background p-6 sm:p-10">
      <div className="relative z-10 max-w-2xl">
        <p className="inline-flex items-center gap-2 rounded-full border bg-background/70 px-3 py-1 text-xs text-muted-foreground backdrop-blur">
          <ShieldCheck className="h-3.5 w-3.5"/> Mobile‑first • Secure • Accessible
        </p>
        <h1 className="mt-4 text-3xl font-extrabold tracking-tight sm:text-4xl">
          Smart Curriculum Activity & Attendance
        </h1>
        <p className="mt-3 text-base text-muted-foreground sm:text-lg">
          Automates attendance via QR, Bluetooth/Wi‑Fi, or Face ID; displays real‑time presence; suggests personalized tasks; and generates integrated daily routines.
        </p>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link to="/dashboard"><Button className="shadow-sm">Start Demo</Button></Link>
          <a href="#features">
